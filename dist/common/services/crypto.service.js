...
let CryptoService = class CryptoService {
    ...
    key = crypto.scryptSync(process.env.ENCRYPTION_KEY || 'peguu-master-key-2026', 'salt', 32);
    encrypt(data) {
    ...
    encryptToBuffer(data) {
        const buffer = Buffer.isBuffer(data) ? data : Buffer.from(data, 'utf8');
        const { encryptedData, iv, authTag } = this.encrypt(buffer);
        return Buffer.concat([iv, authTag, encryptedData]);
    }
    decryptFromBuffer(combined) {
        if (!combined || combined.length < 32) {
            throw new Error(`Datos encriptados inválidos o corruptos. Longitud mínima requerida: 32 bytes, recibida: ${combined?.length || 0}`);
        }
